...
public class PluginManager {
  ...
  public void addPluggableCommands(CommandManager cm) {
    ...
    loadPlugin(cm, null, "");
    for (String plugin : jars.keySet()) {
      loadPlugin(cm, jars.get(plugin), plugin + ":");
    }
  }
  ...
  private void loadPlugin(CommandManager cm, URL jarFile, String prefix) {
    ...
    ServiceLoader<ICommandProvider> serviceLoader =
        ServiceLoader.load(ICommandProvider.class, classLoader);
    for (ICommandProvider service : serviceLoader) {
      for (Command pluggableCommand : service.getCommands()) {
